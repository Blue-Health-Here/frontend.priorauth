        ...
        <div className="absolute right-0 w-40 sm:w-50  border border-light-stroke bg-primary-white rounded-lg shadow-lg z-50" ref={dropdownRef}>
            ...
            <ul className="pb-3">
                {items.map((item, index) => (
                    <li key={index}>
                        <button
                            className="w-full text-left px-4 py-2 text-xs sm:text-sm lg:text-base text-secondary-black hover:text-primary-white hover:bg-primary-sky-blue cursor-pointer"
                            onClick={() => {
                                item.onClick();
                                onClose();
                            }}
