...
const DashboardRequestCard: React.FC<any> = ({
  ...
  // Get chart data with appropriate colors based on mode
  const getChartData = (period: string) => {
    const chartData = JSON.parse(JSON.stringify(baseChartData[period]));
    chartData.datasets[0].backgroundColor = isDarkMode ? '#099DDE' : '#66D0FF';
    return chartData;
  };
  ...
  const updateChart = (period: any) => {
    if (chartInstance.current && !chartInstance.current.destroyed) {
      try {
        chartInstance.current.data = getChartData(period);
  ...
  useEffect(() => {
    if (chartInstance.current && !chartInstance.current.destroyed) {
      chartInstance.current.data = getChartData(activePeriod);
