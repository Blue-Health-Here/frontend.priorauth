...
const CommentsWidget: React.FC<any> = ({ showTwo = false }) => {
    ...
    const handleAdd = async () => {
        ...
        try {
            const response = await addNewReqComment(dispatch, requestId, { comment: input });
            if (response) {
                ...
                <div className="space-y-6 p-3 border-b border-quaternary-navy-blue">
                    <AnimatePresence>
                        {(showTwo ? lastTwo : reqComments).map((c: any) => (
                            ...
                            >
                                <NameBadge data={{ name: c.userName, textColor: '#fff' }} showName={false} />
                                <div className="flex-1 space-y-1">
                                    <p className="font-medium leading-none text-gray-900">{c.userName}</p>
